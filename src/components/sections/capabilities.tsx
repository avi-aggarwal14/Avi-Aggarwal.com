 ...
 * import a React component — keeping the content file editable by someone
 * with no interest in the component tree.
 *
 * All six come from one set (lucide), one size, one stroke weight. Mixing
 * icon sets is one of the loudest tells of an unconsidered interface.
 */
...
/**
 * Capabilities — a grid with a cursor-tracked gold spotlight.
 *
 ...
 * written to a CSS custom property on the node rather than to React state: a
 * pointer-move handler calling `setState` re-renders the whole grid every
 * frame, which is exactly what makes a page with six static cards feel
 * sluggish.
 *
 * Hairlines come from the container showing through 1px gaps, not from a
 * border on every card. Bordered cards separated by a gap stack up to 3px of
 * line between neighbours, which reads heavy next to the 1px rules used
 * everywhere else.
 */
    ...
    >
      ...
      <div
        ...
        style={{
          opacity: active ? 1 : 0,
          background:
            "radial-gradient(340px circle at var(--mx) var(--my), rgba(214,183,124,0.11), transparent 70%)",
        }}
      />
      ...
      <div className="relative">
        <div className="flex items-start justify-between">
          <span className="border-bone/10 bg-ink-raised text-accent group-hover:border-accent/40 inline-flex h-11 w-11 items-center justify-center rounded-lg border transition-colors duration-300">
